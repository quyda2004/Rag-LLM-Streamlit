...
flowchart TD
    ...
    %% --- Nhánh 2/3/4: PHẢI lọc thủ công trước rồi mới xoay
    BR1 -->|2/3/4| M234[Manual QA: kiểm tra/sửa nhãn<br/>(chỉ nhánh 2/3/4)]
    M234 --> ROT1[04_rotated_images.py<br/>Xoay 90/180/270 theo nhãn 2/3/4]
    ROT1 --> OK234[OK loại 2/3/4<br/>Sau xoay lần 1]

    %% --- Nhánh 0: đi vòng lặp 2
    BR1 -->|0| Z0_RAW[Nhánh 0: trang đôi / cần tách]

    %% =========================
    %% VÒNG LẶP 2: CHỈ NHÁNH 0 -> phân loại lại -> lật -> cắt -> gán nhãn -> xoay (nếu cần)
    %% =========================
    Z0_RAW --> CNN0[02_page_intelligence (CNN)<br/>Phân loại lại NHÁNH 0<br/>(xác nhận trang đôi cần tách)]
    CNN0 --> M0A[Manual QA: xác nhận trước khi tách]

    M0A --> FLIP0[Pre-step: lật ngang ảnh<br/>(bắt buộc trước khi cắt)]
    FLIP0 --> SPLIT[05_split_images.py<br/>Cắt dọc giữa → tách 2 trang]
    SPLIT --> Z0_NEW[Ảnh mới sau tách (từ nhánh 0)]

    %% Sau tách: lọc thủ công/gán nhãn, rồi nếu rơi vào 2/3/4 mới xoay
    Z0_NEW --> M0B[Manual QA: lọc lại sau tách<br/>gán nhãn 1/2/3/4]

    M0B --> BR2{Tách nhánh sau tách}
    BR2 -->|1| OK1_FROM0[OK loại 1 (từ nhánh 0)]

    BR2 -->|2/3/4| M0C[Manual QA: kiểm tra/sửa nhãn<br/>(nhánh 2/3/4 sau tách)]
    M0C --> ROT2[04_rotated_images.py<br/>Xoay lại nếu cần]
